            ...
            <div className="space-y-4">
              {profile?.role === 'student' ? (
                upcomingAssignments.length === 0 ? (
                  ...
                  upcomingAssignments.map((act: any, i: number) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-gray-50 rounded-2xl group cursor-pointer hover:bg-gray-100 transition-all border border-transparent hover:border-gray-200 gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 font-medium">Course: {act.course_name} • Due: {act.due_date}</p>
                        </div>
                      </div>
                      <button
              ...
              {announcements.length === 0 ? (
                ...
                announcements.map((ann: any, i: number) => (
                  <div key={i} className="border-l-4 border-black pl-4 py-1">
                    ...
                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest mt-1">Course: {ann.course_name || 'General'}</p>
                  </div>
                ))
              )}
            </div>
              ...
              {upcomingClasses.length === 0 ? (
                ...
                upcomingClasses.map((item: any, idx: number) => (
                  <div key={idx} className="p-4 border border-gray-100 rounded-2xl space-y-2">
                    ...
                    <p className="text-xs text-gray-500 font-medium">Course: {item.course_name || 'Classroom'}</p>
                    <p className="text-xs text-gray-400 font-semibold">{item.scheduled_at}</p>
                    <a
          ...
          <Card title={isAdmin ? 'Unpaid Balance Alerts' : 'Learning Progress'}>
             <div className="space-y-6 mt-4">
               {isAdmin ? (
                 ...
                 <>
                   {enrolledCoursesList.map((course: any, i: number) => {
                     const progress = Math.round(course.progress || 0);
